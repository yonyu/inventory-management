...
const AddOrder = () => {
  ...
  let filteredCategories = newOrder.supplier
    ? products
        .filter((p: any) => p.supplier?._id === newOrder.supplier)
        .map((p: any) => p.category)
        .filter(
          (c: any, i: number, arr: any[]) =>
            c && arr.findIndex((cat: any) => cat?._id === c?._id) === i
        )
    : categories;
  ...
  let filteredProducts = newOrder.category
    ? products.filter(
        (p: any) =>
          p.category?._id === newOrder.category &&
          (!newOrder.supplier || p.supplier?._id === newOrder.supplier)
      )
    : newOrder.supplier
  ...
  const handleAddMoreFormSubmit = async () => {
    ...
    try {
      const ordersToSave = pendingOrders.map(
        ({ tempId, _id, ...order }) => order
      );
      await addOrder(ordersToSave).unwrap();
      setSnackbar({
        open: true,
        message: `${pendingOrders.length} order(s) saved successfully`,
        severity: "success",
      });
      setPendingOrders([]);
    } catch (error: any) {
      setSnackbar({
        open: true,
        message: error?.data?.err || error?.message || "Failed to save orders",
        severity: "error",
      });
    }
  };
  ...
  const handleDeleteOrder = () => {
    if (selectedOrder?.tempId) {
      setPendingOrders(
        pendingOrders.filter((o) => o.tempId !== selectedOrder.tempId)
      );
      setSnackbar({
        open: true,
                ...
                <TableRow key={order.tempId} sx={{ bgcolor: "#fffacd" }}>
                  ...
                  <TableCell>
                    <IconButton
                      onClick={() =>
                        setPendingOrders(
                          pendingOrders.filter((o) => o.tempId !== order.tempId)
                        )
                      }
                    >
                      <Delete sx={{ color: "red" }} />
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth variant="outlined">
                ...
                >
                  {filteredCategories &&
                    filteredCategories?.map((s: any, index: number) => (
                      <MenuItem key={s._id} value={s._id}>
                        {s.name}
                      </MenuItem>
                    ))}
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth variant="outlined">
                ...
                >
                  {filteredProducts &&
                    filteredProducts?.map((s: any, index: number) => (
                      <MenuItem key={s._id} value={s._id}>
                        {s.name}
                      </MenuItem>
                    ))}
            ...
            <Grid size={{ xs: 12 }}>
              <TextField
                ...
                onChange={
                  handlePurchaseOrderNumberChange /* (e) => setNewOrder({ ...newOrder, order_number: e.target.value }) */
                }
                slotProps={{ inputLabel: { style: { color: "white" } } }}
                sx={{
                  mt: 2,

...
const InvoiceTable = ({ search }: { search: string }) => {
  ...
  const {
    ...
    isLoading: loading,
    error,
  } = useGetInvoiceWithDetailsQuery(search, {
    skip: !search,
  });
  ...
  const subTotal =
    invoiceDetails &&
    invoiceDetails?.reduce((acc: number, item: any) => acc + item.totalCost, 0);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error?.data?.err || "Failed to approve the invoice",
        severity: "error",
      });
    }
  };
